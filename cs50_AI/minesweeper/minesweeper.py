...
class Minesweeper():
    ...
    """
    def __init__(self, height=8, width=8, mines=8, seed=None):

        # Set initial width, height, and number of mines
        self.height = height
        self.width = width
        self.mines = set()
        self.seed = random.randrange(10**8, 10**9 - 1) if seed == None else seed
...
class Sentence():
    ...
    def __init__(self, cells, count):
        ...
        self.count = count
        self.coord = tuple() # DEBUGGING PURPOSES
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        2) mark the cell as safe and updates sentences
        ...
        6) draw new inferences that could be make after another
        """
        ...
        # ONLY UNDETERMINATED STATES (with updated count)
        unknown_cells = neighbors - self.mines - self.safes
        new_count = count - len(neighbors & self.mines)

        # Create and append new sentence
        new_sentence = Sentence(unknown_cells, new_count)

        new_sentence.coord = cell # DEBUGGING PURPOSES
        self.knowledge.append(new_sentence)

        def knowledge_based_actions():
            is_inference = False
            
            # 4) Mark additional cells as safe or mine
            #    based on sentence internal knowledge
            # Iterate through without changed knowledge list
            knowledge = self.knowledge.copy()
            for sentence in knowledge:
    
                # Updates mines, safes and blank sentences in AI
            ...
            for sentence_1, sentence_2 in combinations(knowledge, 2):
                ...
                # If there are no subsets or equal sentences in pairs
                else:
                    continue
                ...
                is_inference = True
                print(f"INFERENCE! {new_infer_sentence}")
            
            # 6) If new inference is made I can try make another inference
            if is_inference:
                knowledge_based_actions()
        
        # Makes encapsulated steps 4, 5, 6
        knowledge_based_actions()

        # DEBUGGING PRINTS
        for sentence in self.knowledge:
            print(f"Sentence {sentence.coord}: {sentence}")
        print()
    ...
    def make_random_move(self):
        ...
        # In any other event returns None
        return None
