...
class UI:
    ...
    def input(self):
        ...
        if self.state == 'general':
            ...
            if self.available_monsters:
                ...
                if keys[pygame.K_SPACE]:
                    switched = self.available_monsters[self.switch_index]
                    self.get_input(self.state, switched)
    ...
    def quad_select(self, index, options):
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                ...
                text_surf = self.font.render(options[option], True, color)
    ...
    def switch(self):
        ...
        if self.switch_index < self.visible_monsters:
            ...
            v_offset = (self.visible_monsters - self.switch_index - 1) * rect.height / self.visible_monsters
        for i in range(len(self.available_monsters)):
            ...
            # Icons
            icon_surf = self.simple_surfs[name]
            icon_rect = icon_surf.get_frect(center = (rect.left + 80, y))
...
class OpponentUI:
    def __init__(self, monster):
        self.display_surface = pygame.display.get_surface()
        self.font = pygame.font.Font(None, 30)
        self.left = WINDOW_WIDTH / 2 - 70
        self.top = WINDOW_HEIGHT / 2 - 150
        self.monster = monster
    ...
    def draw(self):
        ...
        # Health bar
        health_rect = pygame.FRect(name_rect.left, name_rect.bottom + 10, rect.width * 0.9, 20)
        ratio = rect.width / self.monster.max_health
